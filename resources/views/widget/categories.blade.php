    ...
    <div class="hot-posts">
        <ul class="list-group shadow">
            @foreach($categories as $category)
                ...
                @endphp
                <a class="{{ $is_current?'bg-light text-success ':'' }}border-0 list-group-item d-flex justify-content-between list-group-item-action" href="{{ route('category.show',$category->name) }}" title="{{ $category->name }}">
